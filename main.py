...
def main():
    ...
    all_results = {}
    models_to_test = {
        "Hybrid": HybridSeparator(in_ch=8, out_ch=4),
        "LSTM": LSTMSeparator(in_ch=8, out_ch=4),
        "Linear": LinearSeparator(in_ch=8, out_ch=4)
    }

    for name, model in models_to_test.items():
        print(f"--- Training {name} ---")
        trained_model, t_hist, v_hist = train_model(model, train_loader, val_loader, epochs=50, device=device)
