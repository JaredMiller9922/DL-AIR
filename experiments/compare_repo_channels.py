...
from utils.model_utils.conversion_helpers import complex_matrix_to_iq_channels, complex_to_2ch
...
def evaluate_checkpoint(checkpoint_path: Path, n_rx: int, phase_shift_deg: int, cfg: EvalConfig, device: str):
    model = IQCNNSeparator(in_ch=2 * n_rx, out_ch=4).to(device)
    ...
    for alpha_idx, alpha in enumerate(cfg.alpha_values):
        for sigma_idx, sigma2 in enumerate(cfg.sigma2_values):
            ...
            for trial in range(cfg.trials):
                ...
                if n_rx == 1:
                    x = complex_to_2ch(ex["mixture"]).astype(np.float32)
                else:
                ...
                rec_a = recover_symbols_from_waveform(pred_a, taps, cfg.sps, len(ex["symbols_a"]))
                rec_b = recover_symbols_from_waveform(pred_b, taps, cfg.sps, len(ex["symbols_b"]))
                true_a = ex["symbols_a"]
                true_b = ex["symbols_b"]
...
def main():
    ...
    cfg = EvalConfig()
    runs = {
        "single_old": (PROJECT_ROOT / "pytorch_models" / "IQ_CNN_1_channel_backup.pt", 1, 5),
        "two_old": (PROJECT_ROOT / "pytorch_models" / "IQ_CNN_2_channel_backup.pt", 2, 5),
        "four_old_phase5": (PROJECT_ROOT / "pytorch_models" / "IQ_CNN_4_channel_backup.pt", 4, 5),
        "four_old_phase15": (PROJECT_ROOT / "pytorch_models" / "IQ_CNN_4_channel_backup.pt", 4, 15),
    }
    out = {"config": asdict(cfg), "device": device, "results": {}}
    for label, (ckpt, n_rx, phase) in runs.items():
        out["results"][label] = evaluate_checkpoint(ckpt, n_rx, phase, cfg, device)
