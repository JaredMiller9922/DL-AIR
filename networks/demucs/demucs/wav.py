...
def _track_metadata(track, sources, normalize=True, ext=EXT):
    ...
    # --- IQ DATA PARAMETERS ---
    # We use these if torchaudio fails to read a header
    IQ_LENGTH = 262144 
    IQ_SAMPLERATE = 1000000 
    # --------------------------
    ...
    for source in sources + [MIXTURE]:
        ...
        if source == MIXTURE and not file.exists():
            # If mixture is missing, we skip trying to build it for now
            # as ta.load might also fail on headerless files
            pass

        try:
            # Try standard audio info check
            info = ta.info(str(file))
            length = info.num_frames
            samplerate = info.sample_rate
            # If header exists but returns 0 frames (common in corrupted files)
            if length == 0:
                length = IQ_LENGTH
                samplerate = IQ_SAMPLERATE
        except Exception:
            # FALLBACK: This is where your raw IQ files will be caught
            length = IQ_LENGTH
            samplerate = IQ_SAMPLERATE
        ...
        if source == MIXTURE and normalize:
            try:
                # If it's IQ data, we use neutral normalization (0 mean, 1 std)
                # because ta.load will likely crash without a header
                if samplerate == IQ_SAMPLERATE:
                    mean = 0.0
                    std = 1.0
                else:
                    wav, _ = ta.load(str(file))
...
def build_metadata(path, sources, normalize=True, ext=EXT):
    ...
    print(f"DEBUG: Scanning path: {path}") # Let's see it in the console
    
    with ThreadPoolExecutor(8) as pool:
        # We search specifically for folders that look like sample folders
        for root, folders, files in os.walk(path, followlinks=True):
            root = Path(root)
            
            # Skip the root path itself and hidden folders
            if root == path or root.name.startswith('.'):
                continue
            
            # Check if this folder contains the mixture file
            # If it does, we treat it as a sample regardless of subfolders
            if (root / f"mixture{ext}").exists():
                name = str(root.relative_to(path))
                pendings.append((name, pool.submit(_track_metadata, root, sources, normalize, ext)))
        
        if not pendings:
            print(f"DEBUG: Found 0 valid samples in {path}!")
        else:
            print(f"DEBUG: Found {len(pendings)} samples. Processing...")
