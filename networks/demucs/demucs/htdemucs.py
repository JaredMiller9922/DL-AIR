...
class HTDemucs(nn.Module):
    ...
    def forward(self, mix):
        if torch.is_complex(mix):
            mix = complex_to_channels(mix)
...
def complex_to_channels(x):
    """Convert complex tensor to real I/Q channels."""
    if torch.is_complex(x):
        real = x.real
        imag = x.imag
        if x.dim() == 2:
...
def channels_to_complex(x):
    """Convert I/Q channels back to complex."""
    real = x[:, :, 0]
    imag = x[:, :, 1]
    return torch.complex(real, imag)

class LearnableChannelizer(nn.Module):
    """Replaces the STFT with a learnable 1D Convolution for RF signals."""
    def __init__(self, in_channels=2, out_channels=512, kernel_size=2048, stride=512):
        super().__init__()
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size, stride=stride)
        self.deconv = nn.ConvTranspose1d(out_channels, in_channels, kernel_size, stride=stride)
...
class HTDemucs(nn.Module):
    """Hybrid Transformer Demucs adapted for RF I/Q signals."""
    def __init__(
        self,
        sources=2,
        audio_channels=2, # I and Q
        channels=48,
        growth=2,
        depth=6,
        transformer=True,
        ...
        # Learnable RF Channelizer (Replaces STFT)
        self.channelizer = LearnableChannelizer(
            in_channels=audio_channels, 
            out_channels=channels * growth ** (depth - 1), 
            kernel_size=nfft, 
            stride=hop_length
        )

        # Time encoder
        self.time_encoder = Demucs(
            sources=sources,
        ...
        if transformer:
            self.transformer = CrossTransformerEncoder(
                dim=channels * growth ** (depth - 1),
                depth=4,
                heads=8,
            )
        else:
        ...
        # Output projection
        self.output = nn.Conv1d(channels, sources * audio_channels, 1)

    def forward(self, mix):
        # 1. Convert complex input to I/Q
        if torch.is_complex(mix):
            mix = complex_to_channels(mix)
        ...
        # 2. Time branch
        time_features = self.time_encoder.encoder(x)
        
        # 3. Frequency/Channelizer branch
        freq_features = self.channelizer(x)

        # 4. Fusion
        if self.transformer is not None:
            # Simplified fusion: pass time_features through transformer
            # (In full HTDemucs, this is cross-attention between time and freq)
            fused = self.transformer(time_features)
        else:
            fused = time_features

        # 5. Decoder
        decoded = self.time_encoder.decoder(fused)

        # 6. Output layer
        out = self.output(decoded)
        out = out.view(B, self.sources, self.audio_channels, -1)

        # Trim or pad to match exact input length (Convolution artifacts)
        if out.shape[-1] != T:
            out = out[..., :T] 

        # 7. Convert back to complex
        out_complex = channels_to_complex(out)
        return out_complex