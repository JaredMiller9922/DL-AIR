    ...
    class Solver(object):
        ...
        def _run_one_epoch(self, epoch, train=True):
            ...
            for idx, sources in enumerate(logprog):
                ...
                # --- MODIFICATION FOR RF DATA ---
                # If your IQ data is complex, convert to (Batch, Source, 2, Time)
                if torch.is_complex(sources):
                    sources = torch.view_as_real(sources).permute(0, 1, 3, 2)
                # --------------------------------
                if train:
