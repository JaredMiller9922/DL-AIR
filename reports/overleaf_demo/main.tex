...
This report summarizes the RF source separation work completed across model comparison, hyperparameter search, regularization, and controlled enclosed experiments. The goals were to identify a strong model family, tune it with broad and focused searches, and verify that the training pipeline can learn progressively more RF-like separation problems under controlled conditions.

The main metric used in the tuning runs was permutation-invariant mean squared error (PIT-MSE), since source ordering may swap while still representing a correct separation. Signal-to-distortion ratio (SDR) is also reported in decibels.

\section{Model Selection for Cross-Validation}
Earlier comparison runs showed that \texttt{IQ\_CNN} and \texttt{LSTM} were the strongest candidates. \texttt{IQ\_CNN} was selected as the main cross-validation target because it slightly outperformed the alternatives while remaining competitive in separation quality.

On this branch, a narrow learning-rate sweep was also run for \texttt{LSTM} and \texttt{IQ\_CNN}. Lower PIT-MSE is better.
...
These runs reinforced the decision to keep \texttt{IQ\_CNN} as the primary cross-validation target. Although \texttt{LSTM} remained competitive, the best \texttt{IQ\_CNN} score was slightly lower.

\section{Broad Cross-Validation}
The broad search was designed as an exploratory pass over a large hyperparameter space for \texttt{IQ\_CNN}. The run used a 12-hour budget, completed 46 trials, and used 5-fold cross-validation over the combined train/validation pool.

\subsection{Broad Search Space}
The broad search varied:
\begin{itemize}
...
This broad search was useful for locating a promising region, but it spread the 12-hour budget over many configurations, so each individual configuration was not trained as deeply as desired.

\subsection{Best Broad Search Result}
The best broad-search trial was Trial 29.
...
After the broad run, the hyperparameter search was narrowed around the best region. The focused deep search fixed the search around \texttt{base\_channels = 48} and used much longer runs, up to 200 epochs per configuration, with validation-based model selection.

\subsection{Best Focused Search Result}
The best focused run was \texttt{deep\_cfg\_03}.
...
The focused search improved clearly over the broad search. Once a good parameter neighborhood had been located, deeper training on fewer configurations produced more useful results than another broad sweep.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/simple_wave/01_sources.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/simple_wave/02_mixture.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/simple_wave/03_outputs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/structured_rf_bridge/00_dataset_randomness.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/structured_rf_bridge/01_clean_sources.png}
    \caption{Clean complex source signals before channel mixing and noise. The real and imaginary parts are shown separately.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/structured_rf_bridge/02_received_mixture.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/structured_rf_bridge/03_model_outputs.png}
    \caption{Separated outputs plotted directly against the clean sources. Close overlap indicates successful separation.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/structured_rf_bridge/04_frequency_view.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/structured_rf_bridge/05_sample_metadata.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/qpsk_rf_step/00_dataset_randomness.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/qpsk_rf_step/01_clean_sources.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/qpsk_rf_step/02_received_mixture.png}
    \caption{Received mixture from two example receivers. This is the signal the model actually receives and must separate.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/qpsk_rf_step/03_model_outputs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/qpsk_rf_step/04_frequency_view.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/qpsk_rf_step/05_sample_metadata.png}
...
The work produced several useful outcomes:
\begin{enumerate}
    \item Model selection was justified. \texttt{IQ\_CNN} and \texttt{LSTM} were the strongest families, and later tuning confirmed that \texttt{IQ\_CNN} was the best focus for deeper search.
    \item Broad cross-validation was useful for exploration. It identified a strong parameter neighborhood around a medium-width \texttt{IQ\_CNN} with moderate dropout and nonzero regularization, but also showed that too-large a search space can dilute training depth.
    \item Focused cross-validation produced a clearly better result. Narrowing the search around the best broad-search region yielded a stronger and more defensible best configuration.
    \item The training pipeline can learn when the structure is controlled. The three enclosed experiments demonstrated consistent learning from very simple waves up through a pulse-shaped QPSK RF-like task.
    \item The controlled experiment ladder was informative. Each experiment added complexity while preserving learnability, creating a bridge from trivial toy signals to more realistic RF-style data.
\end{enumerate}
...
Overall, the results support the conclusion that the project has a viable source-separation pipeline, that the best model family has been identified, and that controlled bridge experiments can help diagnose the gap between easy learnable tasks and the full RF separation problem.
